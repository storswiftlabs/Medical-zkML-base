...
from leo_translate.submodule import Sign
...
def generate_functions_body(n_estimators, n_classes, is_leaves_est, threshold_est, feature_est, children_left_est,
                            children_right_est, values_est, fixed_number, struct_name, context, leo_display_type):
    """
...
def generate_transition_body(context, first_input_arg, result_type, is_classification, n_classes):
    ...
    for func in function_list:
        ...
        transition_body.append(Let(let_variate, result_type, let_variate_body).get())
    if not is_classification:
        ...
        for index in range(n_classes):
            ...
            class_variate_type = result_type
        ...
        transition_body.append(Let(max_ele, result_type, "c0").get())
        transition_body.append(Let(res_variate, result_type, Int_value(0, result_type).get()).get())
        for index in range(1, n_classes):
            left_value = "c" + str(index)
            right_value = max_ele
            sign = Sign.GREATER_THAN.value
            body = f"{max_ele} = {left_value};\n{res_variate} = {str(index)}{result_type};"
            transition_body.append(IfControl(left_value, right_value, sign, body).get())
        transition_body.append(ReturnStatement(res_variate).get())
    return transition_body


def xgboost_leo_code(boost, dc: data_control, is_classification: bool = True, leo_name: str = "main"):
    ...
    for index in range(0, len(boost.feature_importances_)):
        inputs_name_and_type["c" + str(index)] = dc.display_type
    ...
    functions_body = generate_functions_body(n_estimators, n_classes, is_leaves, threshold, feature, children_left,
                                             children_right, values, dc.fixed_number, struct_name, context,
                                             dc.display_type)
    for index, tree in enumerate(functions_body):
        ...
        context.add_function(variate, inputs, dc.display_type, body=tree)
    ...
    input1 = struct_name.lower()
    inputs = f"{input1}{Sign.COLON.value} {struct_name}"
    transition_body = generate_transition_body(context, input1, dc.display_type, is_classification, n_classes)
    context.add_transition(variate, inputs, dc.display_type, transition_body)

    # leo code generate
    leo_code_list = context.generate_leo_code_list(leo_name, dc.fixed_number)
    # code table format
